...
router.get('/summaries', async (req, res) => {
  try {
    ...
    const items = await studySummaries.find({ user_id: req.user.id }).sort({ created_at: -1 }).limit(50);
    return res.json({ items: items.map((doc) => ({ ...(doc.toObject ? doc.toObject() : doc), id: doc._id })) });
  } catch (error) {
...
router.post('/summarize', async (req, res) => {
  try {
    ...
    const generated = generateSummary(text);
...
router.get('/daily-plan', async (req, res) => {
  try {
    const { questionAttempts, flashcards, deadlineReminders, schedule } = getDatabase();

    const attempts = await questionAttempts.find({ user_id: req.user.id }).sort({ created_at: -1 }).limit(120);
    const weakSubjects = topSubjectsByWeakness(attempts);

    const now = new Date().toISOString();
    const dueFlashcards = await flashcards.countDocuments({ user_id: req.user.id, next_review: { $lte: now } });
    const upcomingReminders = await deadlineReminders.find({ user_id: req.user.id, active: true }).sort({ due_at: 1 }).limit(5);
    const weeklySlots = await schedule.countDocuments({ user_id: req.user.id });
