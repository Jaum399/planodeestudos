...
router.post('/', async (req, res) => {
  try {
    ...
    if (!['prova', 'trabalho'].includes(normalizedKind)) {
      return res.status(400).json({ error: 'Tipo inválido. Use: prova ou trabalho' });
    }
    ...
    if (alert_whatsapp && !normalizedAlertWhatsapp) {
      return res.status(400).json({ error: 'Número de alerta inválido. Use DDD + número.' });
    }
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (!['prova', 'trabalho'].includes(updates.kind)) {
      return res.status(400).json({ error: 'Tipo inválido. Use: prova ou trabalho' });
    }
