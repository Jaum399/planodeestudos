...
async function buildSmartExecutionContext(userId) {
  try {
    const { planner, flashcards, deadlineReminders, questionAttempts } = getDatabase();
    const now = new Date().toISOString();

    const [todoCount, dueFlashcards, nextReminder, attempts] = await Promise.all([
      planner.countDocuments({ user_id: userId, status: { $in: ['todo', 'in-progress'] } }),
      flashcards.countDocuments({ user_id: userId, next_review: { $lte: now } }),
      deadlineReminders.findOne({ user_id: userId, active: true, due_at: { $gte: now } }).sort({ due_at: 1 }),
      questionAttempts.find({ user_id: userId }).sort({ created_at: -1 }).limit(40),
    ]);

    const bySubject = new Map();
    for (const attempt of attempts || []) {
      const subject = String(attempt.subject || 'Geral');
      const stat = bySubject.get(subject) || { total: 0, correct: 0 };
      stat.total += 1;
    ...
    let weakest = null;
    for (const [subject, stat] of bySubject.entries()) {
      const accuracy = stat.total > 0 ? Math.round((stat.correct / stat.total) * 100) : 0;
      if (!weakest || accuracy < weakest.accuracy) {
        weakest = { subject, accuracy, total: stat.total };
      }
    }
    ...
    console.warn('[Jarvis] Falha ao montar contexto inteligente:', err.message);
    return null;
  }
}

function buildExecutionHint(smartContext) {
  if (!smartContext) {
    return 'Próxima ação sugerida: inicie com 25 min de foco e 5 min de revisão final.';
  }

  if (smartContext.dueFlashcards > 0) {
    return `Próxima ação sugerida: revise ${smartContext.dueFlashcards} flashcards pendentes antes de abrir novo conteúdo.`;
  }

  if (smartContext.weakest && smartContext.weakest.total >= 6) {
    return `Próxima ação sugerida: foque em ${smartContext.weakest.subject} (acurácia recente ${smartContext.weakest.accuracy}%).`;
  }

  if (smartContext.todoCount > 0) {
    return `Próxima ação sugerida: conclua 1 das ${smartContext.todoCount} tarefas ativas no Planner agora.`;
  }

  if (smartContext.nextReminder?.dueAt) {
    const when = new Date(smartContext.nextReminder.dueAt).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' });
    return `Próxima ação sugerida: prepare-se para "${smartContext.nextReminder.title}" (${when}).`;
  }

  return 'Próxima ação sugerida: faça 20 minutos de revisão ativa para manter consistência.';
}
...
const INTENTS = [
  ...
  { name: 'autopilot',    pattern: /\b(organiza meu dia|organize meu dia|modo autom[aá]tico|autopiloto|fa[cç]a um plano agora|monte meu plano agora)\b/i },
  { name: 'flashcard_generate', pattern: /\b(?:criar?|fazer?|gerar?|montar?)\s+(?:\d+\s+)?flashcards?\b/i },
...
function extractFlashcardRequest(text, fallbackSubject) {
  const safe = String(text || '');
  const match = safe.match(/\b(?:(?:quero|preciso|vou)\s+)?(?:criar?|fazer?|gerar?|montar?)\s+(\d+\s+)?flashcards?\s+(?:de|para|sobre|d[ao]\s+)?([^.,;!?\n]{3,80})/i);

  const quantity = match?.[1] ? Number(match[1].trim()) : 8;
  const theme = String(match?.[2] || '')
    .replace(/\s+(hoje|amanh[aã]|essa\s+semana|agora)\b.*/i, '')
    .trim();

  return {
    quantity: Number.isFinite(quantity) ? quantity : 8,
    theme,
    subject: extractFirstSubject(theme) || fallbackSubject || 'Estudo',
  };
}
...
function buildResponse(intent, text, user, jarvisData, mapData, voiceModel, smartContext) {
  const firstName = user.name.split(' ')[0];
  ...
  const responses = {
    ...
    autopilot: [
      `${openers[flow]} ativei modo execução. Vou priorizar revisão pendente, ponto fraco e tarefa crítica em sequência.`,
      `${openers[flow]} plano automático ligado: primeiro revisão curta, depois ataque de ponto fraco e fechamento com checklist.`,
    ],
    motivation: [
  ...
  const executionHint = buildExecutionHint(smartContext);

  if (model === VOICE_MODELS.manus_ia) {
    return `${modelPrefix[model]}${base} ${executionHint} Responda com uma acao imediata sua em 1 frase e eu refinarei o plano.`;
  }
  if (model === VOICE_MODELS.manus_ia_pro) {
    return `${modelPrefix[model]}${base} ${executionHint} Contexto do seu objetivo: ${trackHint} Me diga seu tempo disponivel hoje e eu te devolvo um plano em 3 blocos.`;
  }
  if (model === VOICE_MODELS.aios) {
    return `${modelPrefix[model]}${base} ${executionHint} Estruture sua execucao em: objetivo, tempo e validacao.`;
  }
  if (model === VOICE_MODELS.aios_coach) {
    return `${modelPrefix[model]}${base} ${executionHint} Framework coach: objetivo claro, tarefa de 25min, criterio de conclusao e revisao final de 5min. Track atual: ${track}.`;
  }
  return `${modelPrefix[model]}${base} ${executionHint}`;
}
  ...
  for (const action of actions) {
    try {
      if (action.type === 'task') {
        ...
        const subject = extractFirstSubject(action.label) || user.area || 'Estudo';
        const generated = await createFlashcardsForTheme({
          userId,
          theme: capitalizeFirst(action.label),
          subject: capitalizeFirst(subject),
          quantity: action.quantity || 8,
        });
        executed.push({ ...action, created: true, quantity: generated.created, deck_id: generated.deck_id });
...
router.post('/chat', async (req, res) => {
  try {
    ...
    const smartContext = await buildSmartExecutionContext(req.user.id);
    ...
    if (nectarMode === true) {
      ...
      if (rawActions.length > 0) {
        ...
        const parts = [];
        const taskTotal = cnt('task');
        const flashcardsTotal = executedActions
          .filter(a => a.created && a.type === 'flashcard')
          .reduce((acc, a) => acc + Number(a.quantity || 0), 0);
        if (taskTotal > 0) parts.push(`${taskTotal} ${taskTotal === 1 ? 'tarefa' : 'tarefas'} no Planner`);
        if (flashcardsTotal > 0) parts.push(`${flashcardsTotal} flashcards criados automaticamente`);
    ...
    if (intent === 'flashcard_generate') {
      const request = extractFlashcardRequest(text, req.user.area);
      if (!request.theme || request.theme.length < 3) {
        reply = 'Me diga o tema dos flashcards, por exemplo: "crie 8 flashcards sobre fisiologia renal".';
      } else {
        const generated = await createFlashcardsForTheme({
          userId: req.user.id,
          theme: request.theme,
        ...
        reply = `Perfeito! Criei ${generated.created} flashcards automaticamente sobre ${generated.theme}.`;
        sideEffect = {
      ...
      if (!reply) {
        reply = buildResponse(intent, text, req.user, data, mapData, selectedVoiceModel, smartContext);
      }
    }
