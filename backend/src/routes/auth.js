...
function resolveFrontendUrlFromRequest(req) {
...
router.post('/register', async (req, res) => {
  try {
    ...
    if (whatsapp && !normalizedWhatsapp) {
      return res.status(400).json({ error: 'WhatsApp inválido. Use formato internacional com código do país (ex: +34641296849).' });
    }
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    await users.findOneAndUpdate(
      { _id: user._id },
      { $set: { reset_token: tokenHash, reset_token_expires: expires, updated_at: new Date().toISOString() } }
    );

    const frontendUrl = resolveFrontendUrlFromRequest(req);
    try {
      await sendPasswordResetEmail(user, token, { frontendUrl });
    } catch (emailError) {
      console.warn('[Email] Reset falhou:', emailError?.message || emailError);
    }
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    const { users } = getDatabase();
    const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
    const user = await users.findOne({ reset_token: tokenHash });
