/**
 * Email utility — Resend API (preferencial) + Nodemailer SMTP (fallback).
 *
 * Environment variables (add no Vercel > Settings > Environment Variables):
 *   RESEND_API_KEY    — chave da API Resend (preferencial)
 *   RESEND_FROM       — remetente validado na Resend (ex: AppMentoria <noreply@seu-dominio.com>)
 ...
 *   EMAIL_FROM_NAME    — nome exibido     (padrão: AppMentoria)
 *
 * Se RESEND_API_KEY e SMTP não estiverem configurados, os emails são suprimidos
 * com aviso no log — a aplicação NUNCA quebra por falta de email.
 */
...
function getFrontendUrl(fallbackUrl) {
  return normalizeUrl(process.env.FRONTEND_URL || fallbackUrl || DEFAULT_FRONTEND_URL);
}

function getSmtpUser() {
  return process.env.EMAIL_USER || process.env.SMTP_USER || DEFAULT_EMAIL_USER;
}
...
function getResendFrom() {
  ...
  const name = process.env.EMAIL_FROM_NAME || 'AppMentoria';
  if (user) return `"${name}" <${user}>`;
  return '';
}

function getReplyToAddress() {
  const configured = String(process.env.EMAIL_REPLY_TO || '').trim();
  if (configured) return configured;
...
async function sendEmail(to, subject, html) {
  ...
  if (resendKey) {
    try {
      ...
      if (!fromAddress) {
        throw new Error('RESEND_FROM não configurado e EMAIL_USER indisponível para fallback de remetente.');
      }
      ...
      const emailId = body?.id || body?.message_id || 'unknown';
      console.log(`[Email][Resend] Enviado para ${Array.isArray(to) ? to.join(', ') : to}: ${emailId}`);
      return { provider: 'resend', id: emailId, raw: body };
    } catch (resendError) {
      console.warn('[Email][Resend] Falha no envio, tentando SMTP fallback:', resendError?.message || resendError);
    }
  }

  const transporter = createTransporter();
  if (!transporter) {
    console.warn(`[Email] Provedor indisponível (Resend/SMTP) — pulando envio para: ${to}`);
    return { skipped: true };
  }
  const info = await transporter.sendMail({
    ...
    headers: {
      ...
      'X-Email-Category': 'transactional',
    },
  });
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:580px;background:#18181b;border-radius:16px;border:1px solid #27272a;overflow:hidden;">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#7c3aed,#4f46e5);padding:32px 36px;text-align:center;">
              <span style="font-size:28px;font-weight:800;color:#fff;letter-spacing:-0.5px;">🎓 AppMentoria</span>
            </td>
          </tr>
          <!-- Body -->
          <tr>
          ...
          <tr>
            <td style="background:#09090b;padding:20px 36px;border-top:1px solid #27272a;text-align:center;">
              <p style="margin:0;font-size:12px;color:#71717a;">
                AppMentoria &mdash; <a href="${appUrl}" style="color:#a78bfa;text-decoration:none;">acessar plataforma</a>
              </p>
...
async function sendWelcomeEmail(user) {
  ...
  const body = `
    ...
    <p style="margin:0 0 16px;font-size:15px;color:#a1a1aa;line-height:1.6;">
      Sua conta foi criada com sucesso. Para liberar todos os recursos da plataforma,
      conclua a assinatura com pagamento confirmado.
    </p>
  ...
  `;
  return sendEmail(user.email, '🎓 Bem-vindo(a) ao AppMentoria!', baseTemplate('Bem-vindo!', body));
}
...
async function sendPaymentConfirmationEmail(user) {
  ...
  `;
  return sendEmail(user.email, '✅ Premium ativado — AppMentoria', baseTemplate('Premium ativado!', body));
}

async function sendPasswordResetEmail(user, token, options = {}) {
  const appUrl = getFrontendUrl(options.frontendUrl);
  ...
  const body = `
    ...
    <p style="margin:0 0 16px;font-size:15px;color:#a1a1aa;line-height:1.6;">
      ...
      Clique no botão abaixo para criar uma nova senha. O link é individual, único e expira em <strong style="color:#a78bfa;">1 hora</strong>.
    </p>
  ...
  `;
  return sendEmail(user.email, '🔒 Redefinição de senha — AppMentoria', baseTemplate('Redefinição de senha', body));
}
