...
import { formatCpf, formatWhatsappBr, normalizeEmail, onlyDigits, safeReadJson, safeWriteJson } from '../utils/formAutomation';

const REGISTER_DRAFT_KEY = 'ordex.register.draft';

function loadRegisterDraft() {
  return safeReadJson<{ name: string; email: string; cpf: string; area: string; whatsapp: string }>(REGISTER_DRAFT_KEY, {
    name: '', email: '', cpf: '', area: '', whatsapp: '',
  });
}
...
export default function Register() {
  ...
  useEffect(() => {
    safeWriteJson(REGISTER_DRAFT_KEY, {
      name,
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await register(name.trim(), normalizeEmail(email), password, cleanCpf, area, cleanWhatsapp);
    ...
    <div className="min-h-screen bg-app-bg flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="card-glass rounded-2xl p-8 card-glow">
          ...
          <p className="text-gray-400 text-sm public-subheading mb-7">Conclua o cadastro e assine para liberar o acesso</p>

          {error && (
            <div role="alert" aria-live="assertive" className="bg-red-500/10 border border-red-500/30 rounded-xl px-4 py-3 text-red-400 text-sm mb-5">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate aria-busy={isLoading}>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  required
                  aria-invalid={!!error}
                  aria-describedby={`register-password-help${error ? ' register-form-error' : ''}`}
              ...
              <p id="register-password-help" className="text-[11px] text-gray-600 mt-1">Use pelo menos 6 caracteres.</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                inputMode="numeric"
                aria-invalid={!!error}
                aria-describedby={`register-cpf-help${error ? ' register-form-error' : ''}`}
                autoComplete="off"
              />
              <p id="register-cpf-help" className="text-[11px] text-gray-600 mt-1">Digite apenas os 11 dígitos do CPF ou com máscara.</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                inputMode="tel"
                aria-invalid={!!error}
                aria-describedby={`register-whatsapp-help${error ? ' register-form-error' : ''}`}
                autoComplete="tel"
              />
              <p id="register-whatsapp-help" className="text-[11px] text-gray-600 mt-1">Usado para alertas importantes de provas e trabalhos.</p>
            </div>
            ...
            {error && <span id="register-form-error" className="sr-only">{error}</span>}
          </form>
