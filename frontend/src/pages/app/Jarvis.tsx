...
export default function JarvisPage() {
  ...
  // ── Load Initial State ──────────────────────────────────────────────────────
  useEffect(() => {    mindmapApi.get().then(({ data }) => setMapData(data)).catch(() => {});    jarvisApi.getState()
      .then(({ data }) => {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      const { data } = await jarvisApi.chat(trimmed, { voiceModel, nectarMode });
      ...
      if (data.sideEffect?.type === 'mindmap_updated' && data.sideEffect.map) {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col">
      {/* Header */}
      <div className="px-4 md:px-6 py-4 border-b border-app-border flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className="px-3 py-2 rounded-xl border border-app-border bg-app-card text-gray-300 text-sm focus:outline-none focus:border-cyan-500/40"
            title={t('ai_model')}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm transition-colors ${conversationMode ? 'border-cyan-500/50 text-cyan-300 bg-cyan-500/10' : 'border-app-border text-gray-400 hover:text-white hover:bg-white/5'}`}
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm transition-colors ${showMap ? 'border-purple-500/50 text-purple-300 bg-purple-500/10' : 'border-app-border text-gray-400 hover:text-white hover:bg-white/5'}`}
            title={t('map')}
          >
            <Map size={15} />
            {t('map')}
          ...
          <button
            ...
            className="relative flex items-center gap-2 px-3 py-2 rounded-xl border border-app-border text-gray-400 hover:text-white hover:bg-white/5 transition-colors text-sm"
          >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {/* Input */}
          <div className="px-4 md:px-6 py-4 border-t border-app-border">
        ...
        {/* Reminders Panel */}
        {showReminders && (
          <div className="w-72 border-l border-app-border flex flex-col">
