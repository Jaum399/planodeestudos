...
import { Bell, Calendar, CalendarClock, CheckCircle2, Edit3, Loader2, Plus, Save, Trash2, XCircle } from 'lucide-react';
import { reminderSessionApi } from '../../services/api';
import { addReminderToCalendar, removeReminderFromDevice, syncAllActiveRemindersToDevice, syncReminderToDevice } from '../../services/deviceReminders';
...
export default function ReminderSessionPage() {
  ...
  const [kind, setKind] = useState<'prova' | 'trabalho' | 'apresentacao'>('prova');
  const [dueAt, setDueAt] = useState('');
  const [alertWhatsapp, setAlertWhatsapp] = useState('');

  const [editingId, setEditingId] = useState<string | null>(null);
  const [editingTitle, setEditingTitle] = useState('');
  const [editingKind, setEditingKind] = useState<'prova' | 'trabalho' | 'apresentacao'>('prova');
      ...
      <section className="card-glass rounded-2xl p-6 card-glow">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">Cadastre provas, trabalhos e apresentações para receber alertas e adicionar cada data ao calendário.</p>
          </div>
        </div>
        ...
        <form onSubmit={onCreate} className="grid md:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <select className="input-field" value={kind} onChange={(e) => setKind(e.target.value as 'prova' | 'trabalho' | 'apresentacao')}>
          ...
          <div className="space-y-6">
            <div className="space-y-3">
              ...
              <div key={item.id} className="rounded-xl border border-app-border bg-app-card/50 p-4 flex flex-col md:flex-row md:items-center gap-3">
                  ...
                  {editingId === item.id ? (
                    <div className="space-y-2">
                      ...
                      <div className="grid sm:grid-cols-3 gap-2">
                        <select className="input-field" value={editingKind} onChange={(e) => setEditingKind(e.target.value as 'prova' | 'trabalho' | 'apresentacao')}>
                          <option value="prova">Prova</option>
                          <option value="trabalho">Trabalho</option>
                          <option value="apresentacao">Apresentação</option>
                        </select>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <button
                    onClick={() => void addReminderToCalendar(item)}
                    className="inline-flex items-center gap-1 px-3 py-2 text-xs rounded-lg border border-cyan-500/30 text-cyan-300 hover:bg-cyan-500/10"
