...
function getKindLabel(kind: DeadlineReminder['kind']) {
  if (kind === 'prova') return 'Prova';
  if (kind === 'apresentacao') return 'Apresentação';
  return 'Trabalho';
}
...
function normalizeReminderTitle(reminder: DeadlineReminder) {
  return (reminder.title || 'Lembrete').replace(/\s+/g, ' ').trim();
}

function buildGoogleCalendarUrl(reminder: DeadlineReminder): string {
  const dueAt = new Date(reminder.due_at);
  const start = new Date(dueAt.getTime() - 60 * 60 * 1000);
  const end = dueAt;
  const details = encodeURIComponent(`Ordex - ${reminder.kind}`);
  const title = encodeURIComponent(normalizeReminderTitle(reminder));
...
function downloadIcs(reminder: DeadlineReminder) {
  const dueAt = new Date(reminder.due_at);
  const start = new Date(dueAt.getTime() - 60 * 60 * 1000);
  const formatIcsDate = (value: Date) => value.toISOString().replace(/[-:]/g, '').split('.')[0] + 'Z';
  const content = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'BEGIN:VEVENT',
    `UID:${reminder.id}@ordex`,
    `DTSTAMP:${formatIcsDate(new Date())}`,
    `DTSTART:${formatIcsDate(start)}`,
    `DTEND:${formatIcsDate(dueAt)}`,
    `SUMMARY:${normalizeReminderTitle(reminder)}`,
    `DESCRIPTION:Ordex - ${reminder.kind}`,
    'END:VEVENT',
    'END:VCALENDAR',
  ].join('\r\n');
  ...
  anchor.href = url;
  anchor.download = `${normalizeReminderTitle(reminder).replace(/[^a-z0-9]+/gi, '-').toLowerCase() || 'ordex-lembrete'}.ics`;
  anchor.click();
...
async function upsertCalendarEvent(reminder: DeadlineReminder, _previousEventId?: string | number) {
  if (!isNativeMobile()) {
    const target = window.open(buildGoogleCalendarUrl(reminder), '_blank', 'noopener,noreferrer');
    if (!target) {
...
export async function addReminderToCalendar(reminder: DeadlineReminder) {
  await upsertCalendarEvent(reminder);
}
...
export async function syncReminderToDevice(reminder: DeadlineReminder) {
  ...
  if (triggers.d7) {
    notifications.push({
      id: ids.d7,
      title: `${getKindLabel(reminder.kind)} em 7 dias`,
      body: reminder.title,
  ...
  if (triggers.d2) {
    notifications.push({
      id: ids.d2,
      title: `${getKindLabel(reminder.kind)} em 2 dias`,
      body: reminder.title,
  ...
  if (triggers.due) {
    notifications.push({
      id: ids.due,
      title: `${getKindLabel(reminder.kind)} hoje`,
      body: reminder.title,
