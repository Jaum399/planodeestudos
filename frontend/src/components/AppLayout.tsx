...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  async function speakLoginGreetingAndImportantReminders() {
    if (typeof window === 'undefined' || !('speechSynthesis' in window) || !user) return;

    const voiceEnabled = localStorage.getItem('app.voiceEnabled');
    if (voiceEnabled === 'false') return;

    const loginEvent = sessionStorage.getItem('mentoria_login_event');
    if (!loginEvent) return;
    const handled = sessionStorage.getItem('mentoria_login_event_handled');
    ...
    try {
      ...
      sessionStorage.setItem('mentoria_login_event_handled', loginEvent);
    } catch {
